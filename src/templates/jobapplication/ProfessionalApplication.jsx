...
export default function ProfessionalApplication({ values }) {
  ...
  return (
    ...
    >
      {/* ── Header band ───────────────────────────────────────────────── */}
      <header className="relative overflow-hidden bg-slate-900 px-14 pb-12 pt-14 text-white">
        <p className="text-[12px] font-semibold uppercase tracking-[0.32em] text-indigo-300">
          {t('template.jobapplication.application', 'Job Application')}
        </p>
        <h1
          data-pdf-color="#a5b4fc"
          className="mt-3 bg-gradient-to-r from-white via-indigo-200 to-sky-200 bg-clip-text font-display text-[46px] font-extrabold leading-tight tracking-tight text-transparent"
        >
          {name}
        </h1>
        <p className="mt-2 text-[15px] text-slate-300">{email}</p>
      ...
      <section className="px-14">
        <div className="-mt-7 flex items-center gap-4 rounded-2xl bg-white px-7 py-5 shadow-xl shadow-slate-200 ring-1 ring-slate-100">
          <div className="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-gradient-to-br from-indigo-600 to-violet-700 text-white">
          ...
          <div>
            ...
            <p className="font-display text-[22px] font-bold text-slate-900">{role}</p>
          </div>
        </div>
      </section>

      <div className="flex-1 px-14 pt-10">
        {/* ── Skills (two columns) ────────────────────────────────────── */}
        <section>
          <h2 className="border-b-2 border-slate-900 pb-1.5 text-[13px] font-bold uppercase tracking-[0.2em] text-slate-900">
          ...
          {skills.length ? (
            <div className="mt-4 grid grid-cols-2 gap-x-10 gap-y-2.5">
              {[skillColA, skillColB].map((col, c) => (
                <ul key={c} className="space-y-2.5">
                  {col.map((skill, i) => (
