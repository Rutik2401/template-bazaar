...
export default function StoreInauguration({ values }) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-gradient-to-b from-[#fbf6ea] via-[#f7f1e3] to-[#efe5cf]" />
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_rgba(180,140,60,0.10),_transparent_60%)]" />
      ...
      {/* ── Content ─────────────────────────────────────────────────────── */}
      <div className="relative flex h-full flex-col items-center px-16 pt-20 pb-16 text-center">
        {/* diya / oil lamp inauguration motif */}
        <div className="relative grid place-items-center">
          {/* flame glow */}
          <div className="absolute -top-3 h-14 w-10 rounded-full bg-amber-300/40 blur-md" />
          <svg viewBox="0 0 80 70" className="h-24 w-28 text-[#bfa05a]" fill="none">
            {/* flame */}
        ...
        {/* headline */}
        <h1 className="mt-5 font-display text-[46px] font-bold leading-[1.05] tracking-wide text-[#7a5a22]">
          {t('template.common.inauguration', 'Inauguration')}
        </h1>

        {/* divider */}
        <div className="mt-6 flex items-center gap-3 text-[#bfa05a]">
          <span className="h-px w-14 bg-[#bfa05a]/60" />
        ...
        <h2 className="mt-3 font-display text-[36px] font-semibold leading-tight text-[#5a4423]">{shop}</h2>
        <p className="mt-2 font-serif text-[17px] italic text-[#8a6a30]">{owner}</p>

        {/* details */}
        <div className="mt-auto w-full">
          <div className="mx-auto h-px w-40 bg-[#bfa05a]/50" />
          <p className="mt-6 font-sans text-[11px] font-semibold uppercase tracking-[0.35em] text-[#a8873f]">
            {t('template.common.date', 'Date')}
          </p>
          <p className="mt-1 font-display text-[24px] font-semibold text-[#5a4423]">{date}</p>
