...
function PhotoFrame({ className = '' }) {
  ...
  return (
    <svg viewBox="0 0 180 220" className={className} fill="none" stroke="currentColor" strokeWidth="2">
      <rect x="4" y="4" width="172" height="212" rx="14" fill="currentColor" fillOpacity="0.06" />
      <rect x="12" y="12" width="156" height="196" rx="9" strokeDasharray="5 6" opacity="0.6" />
      {/* portrait silhouette */}
      <circle cx="90" cy="92" r="34" fill="currentColor" fillOpacity="0.16" stroke="none" />
      <path d="M40 188c0-30 22-50 50-50s50 20 50 50" fill="currentColor" fillOpacity="0.16" stroke="none" />
    </svg>
  )
}
...
function Row({ label, value }) {
  return (
    <div className="flex gap-4 py-[7px]">
      <span className="w-36 shrink-0 text-[14px] font-semibold text-rose-700">{label}</span>
      <span className="text-[15px] leading-snug text-slate-700">{value}</span>
    </div>
  )
}
  ...
  return (
    ...
    >
      ...
      <div className="relative h-[300px] bg-gradient-to-br from-rose-100 via-[#fdf2f5] to-amber-50">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,_rgba(244,114,182,0.18),_transparent_55%)]" />
        <p className="absolute left-12 top-12 text-[15px] tracking-wide text-rose-500">॥ श्री गणेशाय नमः ॥</p>

        {/* photo frame, top-right */}
        <div className="absolute right-12 top-12 rounded-2xl bg-white p-2 shadow-[0_18px_50px_-22px_rgba(190,24,93,0.45)]">
        ...
        <div className="absolute bottom-12 left-12 max-w-[58%]">
          ...
          <p className="mt-2 text-[15px] text-slate-500">{occupation}</p>
        </div>
      </div>
      ...
      <div className="flex flex-1 flex-col px-12 pb-12 pt-9">
        <section>
          <h2 className="mb-2 flex items-center gap-2 text-[13px] font-bold uppercase tracking-[0.2em] text-rose-700">
            <span className="h-4 w-1 rounded-full bg-rose-500" />
            {t('template.common.personalDetails', 'Personal Details')}
          </h2>
          <div className="grid grid-cols-2 gap-x-8 rounded-xl bg-rose-50/60 px-6 py-3">
        ...
        <section className="mt-7">
          ...
          <div className="space-y-1.5 px-2">
            {family.map((f, i) => (
              <p key={i} className="flex gap-2 text-[15px] text-slate-700">
                <span className="mt-[9px] h-1.5 w-1.5 shrink-0 rounded-full bg-rose-300" />
                <span>{f}</span>
              </p>
            ))}
          </div>
        ...
        <section className="mt-auto rounded-xl bg-gradient-to-r from-rose-500 to-rose-400 px-6 py-4 text-white">
